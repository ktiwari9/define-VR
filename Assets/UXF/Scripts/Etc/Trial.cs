    ...
    public class Trial
    {
        ...
        public void Pause()
        {
            ...
            else
            {
                ...
                paused = true;
                session.onPause.Invoke(this);
            }
        }

        /// <summary>
        /// Ends the Trial, queues up saving results to output file, stops and saves tracked object data, and stores highscores.
        /// </summary>
        public void End()
        {
            ...
            // Check results against current highscores and update as needed
            if(session.headerToHighscore != "")
            {
                try
                {
                    session.highScoreController.CompareHighScores(session.ppid, Convert.ToSingle(result[session.headerToHighscore]));
                }
                catch
                { /* Just ignore */ }
            }
