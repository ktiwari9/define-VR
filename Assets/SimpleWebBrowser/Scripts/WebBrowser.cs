...
namespace SimpleWebBrowser
{
  ...
  StartCoroutine(          _mainEngine.InitPlugin(Width, Height, MemoryFile, InitialURL,EnableWebRTC,EnableGPU));
            ...
            _mainEngine.OnPageLoaded += OnPageLoaded;
        ...
        public void show(bool useSeparateCamera)
        {
            if (useSeparateCamera)
            {
        ...
        public void hide(bool useSeparateCamera)
        {
            if (useSeparateCamera)
            {
        ...
        public Vector2 GetScreenCoords(Ray ray)
        {
            RaycastHit hit;
        ...
        public void SendMouseButtonEvent(int x, int y, MouseButton btn, MouseEventType type)
        {
            MouseMessage msg = new MouseMessage
            {
                Type = type,
        ...
        public void ProcessScrollInput(int px, int py, float scroll)
        {
            scroll = scroll * _mainEngine.BrowserTexture.height;
        ...
        public ref BrowserEngine getMainEngine()
        {
            return ref _mainEngine;
        }
        ...
        void Update()
        {
            ...
            if (UIEnabled)
            {
                if (!mainUIPanel.UrlField.isFocused) //keys
                {
                    foreach (char c in Input.inputString)
                    {
                        if (c == '\n')
                        {
                            _mainEngine.SendCharEvent((int)c, KeyboardEventType.Down);
                        }
                        else if(c == '\b')
                        {
                            _mainEngine.SendCharEvent((int)c, KeyboardEventType.Down);
        ...
        {
            // Handled in inputstring, no need for duplicate handling
            if(code == KeyCode.Backspace || code == KeyCode.Return)
            {
                return;
            }
